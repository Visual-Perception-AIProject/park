...
print("4. 'c' 키: 현재 찍고 있는 점 초기화")
print("5. 'q' 키: 완료 및 저장 후 종료")
...
while True:
    ...
    if key == ord('t') or key == ord('s'):
        if len(current_points) < 3:
            print("최소 3개 이상의 점을 찍어야 합니다.")
            continue
        
        is_table = (key == ord('t'))
        label = f"T{t_count:02d}" if is_table else f"S{s_count:02d}"
